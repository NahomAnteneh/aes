#include "aes.hpp"
#include <stdexcept>
#include <algorithm>

// S-box lookup table
static constexpr std::array<uint8_t, 256> SBOX = {
    };

// Inverse S-box lookup table
static constexpr std::array<uint8_t, 256> INV_SBOX = {
    };

// Round constants
static constexpr std::array<uint8_t, 10> RCON = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

AES::AES(const std::array<uint8_t, KEY_SIZE> &key)
{
    keyExpansion(key);
}

void AES::keyExpansion(const std::array<uint8_t, KEY_SIZE> &key)
{
    // Copy the initial key
    roundKeys[0] = key;

    // Generate round keys
    for (size_t i = 1; i <= NUM_ROUNDS; ++i)
    {
        generateRoundKey(i);
    }
}

void AES::generateRoundKey(size_t round)
{
    auto &prevKey = roundKeys[round - 1];
    auto &newKey = roundKeys[round];

    // Rotate last word
    std::array<uint8_t, 4> temp = {
        prevKey[13], prevKey[14], prevKey[15], prevKey[12]};

    // Apply S-box to rotated word
    for (uint8_t &byte : temp)
    {
        byte = SBOX[byte];
    }

    // XOR with round constant
    temp[0] ^= RCON[round - 1];

    // Generate new round key
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            size_t idx = i * 4 + j;
            newKey[idx] = (i == 0) ? prevKey[idx] ^ temp[j]
                                   : prevKey[idx] ^ newKey[idx - 4];
        }
    }
}

std::vector<uint8_t> AES::encrypt(const std::vector<uint8_t> &plaintext)
{
    if (plaintext.empty())
    {
        throw std::runtime_error("Empty plaintext");
    }

    // Calculate padding size (PKCS#7)
    uint8_t paddingSize = BLOCK_SIZE - (plaintext.size() % BLOCK_SIZE);

    // Create padded plaintext
    std::vector<uint8_t> padded = plaintext;
    padded.resize(plaintext.size() + paddingSize, paddingSize);

    std::vector<uint8_t> ciphertext;
    ciphertext.reserve(padded.size());

    // Process each block
    for (size_t offset = 0; offset < padded.size(); offset += BLOCK_SIZE)
    {
        // Create state from input block
        AES::State state = createState(padded, offset);

        // Initial round
        addRoundKey(state, roundKeys[0]);

        // Main rounds
        for (size_t round = 1; round < NUM_ROUNDS; ++round)
        {
            subBytes(state);
            shiftRows(state);
            mixColumns(state);
            addRoundKey(state, roundKeys[round]);
        }

        // Final round (no mixColumns)
        subBytes(state);
        shiftRows(state);
        addRoundKey(state, roundKeys[NUM_ROUNDS]);

        // Append encrypted block to result
        auto encrypted = stateToVector(state);
        ciphertext.insert(ciphertext.end(), encrypted.begin(), encrypted.end());
    }

    return ciphertext;
}

std::vector<uint8_t> AES::decrypt(const std::vector<uint8_t> &ciphertext)
{
    if (ciphertext.empty() || ciphertext.size() % BLOCK_SIZE != 0)
    {
        throw std::runtime_error("Invalid ciphertext size");
    }

    std::vector<uint8_t> plaintext;
    plaintext.reserve(ciphertext.size());

    // Process each block
    for (size_t offset = 0; offset < ciphertext.size(); offset += BLOCK_SIZE)
    {
        // Create state from input block
        AES::State state = createState(ciphertext, offset);

        // Initial round
        addRoundKey(state, roundKeys[NUM_ROUNDS]);

        // Main rounds (in reverse)
        for (size_t round = NUM_ROUNDS - 1; round > 0; --round)
        {
            invShiftRows(state);
            invSubBytes(state);
            addRoundKey(state, roundKeys[round]);
            invMixColumns(state);
        }

        // Final round
        invShiftRows(state);
        invSubBytes(state);
        addRoundKey(state, roundKeys[0]);

        // Append decrypted block to result
        auto decrypted = stateToVector(state);
        plaintext.insert(plaintext.end(), decrypted.begin(), decrypted.end());
    }

    // Remove PKCS#7 padding
    if (!plaintext.empty())
    {
        uint8_t paddingSize = plaintext.back();
        if (paddingSize <= BLOCK_SIZE && paddingSize <= plaintext.size())
        {
            // Verify padding
            bool validPadding = true;
            for (size_t i = 0; i < paddingSize && validPadding; ++i)
            {
                if (plaintext[plaintext.size() - 1 - i] != paddingSize)
                {
                    validPadding = false;
                }
            }

            if (validPadding)
            {
                plaintext.resize(plaintext.size() - paddingSize);
            }
        }
    }

    return plaintext;
}

// Helper function implementations
AES::State AES::createState(const std::vector<uint8_t> &input, size_t offset)
{
    State state;
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            state[j][i] = input[offset + i * 4 + j];
        }
    }
    return state;
}

std::vector<uint8_t> AES::stateToVector(const State &state)
{
    std::vector<uint8_t> output(BLOCK_SIZE);
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            output[i * 4 + j] = state[j][i];
        }
    }
    return output;
}

// We still need to implement these core functions:
void AES::addRoundKey(State &state, const RoundKey &key)
{
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j)
        {
            state[i][j] ^= key[i * 4 + j];
        }
    }
}

void AES::subBytes(State &state)
{
    for (auto &row : state)
    {
        for (uint8_t &byte : row)
        {
            byte = SBOX[byte];
        }
    }
}

void AES::shiftRows(State &state)
{
    // Row 1: shift left by 1
    std::rotate(state[1].begin(), state[1].begin() + 1, state[1].end());
    // Row 2: shift left by 2
    std::rotate(state[2].begin(), state[2].begin() + 2, state[2].end());
    // Row 3: shift left by 3
    std::rotate(state[3].begin(), state[3].begin() + 3, state[3].end());
}

uint8_t AES::gmul(uint8_t a, uint8_t b)
{
    uint8_t p = 0;
    uint8_t hi_bit_set;
    for (size_t i = 0; i < 8; ++i)
    {
        if (b & 1)
            p ^= a;
        hi_bit_set = (a & 0x80);
        a <<= 1;
        if (hi_bit_set)
            a ^= 0x1b; // AES irreducible polynomial
        b >>= 1;
    }
    return p;
}

void AES::mixColumns(State &state)
{
    for (size_t i = 0; i < 4; ++i)
    {
        uint8_t s0 = state[0][i];
        uint8_t s1 = state[1][i];
        uint8_t s2 = state[2][i];
        uint8_t s3 = state[3][i];

        state[0][i] = gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3;
        state[1][i] = s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3;
        state[2][i] = s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3);
        state[3][i] = gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3);
    }
}

// Inverse operations
void AES::invSubBytes(State &state)
{
    for (auto &row : state)
    {
        for (uint8_t &byte : row)
        {
            byte = INV_SBOX[byte];
        }
    }
}

void AES::invShiftRows(State &state)
{
    // Row 1: shift right by 1
    std::rotate(state[1].begin(), state[1].end() - 1, state[1].end());
    // Row 2: shift right by 2
    std::rotate(state[2].begin(), state[2].end() - 2, state[2].end());
    // Row 3: shift right by 3
    std::rotate(state[3].begin(), state[3].end() - 3, state[3].end());
}

void AES::invMixColumns(State &state)
{
    for (size_t i = 0; i < 4; ++i)
    {
        uint8_t s0 = state[0][i];
        uint8_t s1 = state[1][i];
        uint8_t s2 = state[2][i];
        uint8_t s3 = state[3][i];

        state[0][i] = gmul(0x0e, s0) ^ gmul(0x0b, s1) ^ gmul(0x0d, s2) ^ gmul(0x09, s3);
        state[1][i] = gmul(0x09, s0) ^ gmul(0x0e, s1) ^ gmul(0x0b, s2) ^ gmul(0x0d, s3);
        state[2][i] = gmul(0x0d, s0) ^ gmul(0x09, s1) ^ gmul(0x0e, s2) ^ gmul(0x0b, s3);
        state[3][i] = gmul(0x0b, s0) ^ gmul(0x0d, s1) ^ gmul(0x09, s2) ^ gmul(0x0e, s3);
    }
}